Pod::Spec.new do |s|
  s.name             = "KeyboardAttachedView"
  s.version          = "1.0.0"
  s.summary          = "控件跟随键盘同步移动"
  s.description      = <<-DESC
                       It is a marquee view used on iOS, which implement by Objective-C.                       DESC
  s.homepage         = "https://github.com/378645852/KeyboardAttachedView.git"
  # s.screenshots      = "www.example.com/screenshots_1.gif", "www.example.com/screenshots_2.gif"
  s.license          = 'MIT'
  s.author           = { “杨德浩” => “378645852@qq.com” }
  s.source           = { :git => "https://github.com/378645852/KeyboardAttachedView.git", :tag => s.version.to_s }  # s.social_media_url = 'https://twitter.com/NAME'

  s.platform     = :ios, ‘6.0’
  # s.ios.deployment_target = '5.0'
  # s.osx.deployment_target = '10.7'
  s.requires_arc = true

  s.source_files = 'KeyboardAttachedView/*'
  # s.resources = 'Assets'

  # s.ios.exclude_files = 'Classes/osx'
  # s.osx.exclude_files = 'Classes/ios'
  # s.public_header_files = 'Classes/**/*.h'
  s.frameworks = 'Foundation'
end